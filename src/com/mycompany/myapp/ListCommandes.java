 ...
 private static final String STRIPE_SECRET_KEY = "sk_test_51MekuZBuy83JycrRFwccFuthQTjhGfDUOGsgImRUeUxYnbUNLkNivEA10Wm6YI1b4LfmmFc37TCZs6jkv4ERDckf00mCAL6jCe";
    ...
    public ListCommandes(Resources res ) {
       ...
       Component.setSameSize(radioContainer, s1, s2);
        ...
        RadioButton partage = RadioButton.createToggle("Commandes", barGroup);
...
Label lblDate = new Label("date : "+dateFormat.format(rec.getDate_commande()),"NewsTopLine2");
    
    Label lblProduit = new Label("Nom de Produit : "+rec.getNom_produit(),"NewsTopLine2");
    
    Label lblPrix = new Label("Prix "+Float.toString(rec.getPrix_produit()),"NewsTopLine2");
    ...
    // Call the delete command route
    ConnectionRequest request = new ConnectionRequest();
    request.setUrl(Statics.BASE_URL + "/deleteCommandejson/" + rec.getId_commande());
    request.setHttpMethod("DELETE");
    request.addResponseListener(ev -> {
        // Show a success message and update the UI
        ToastBar.showInfoMessage("Commande deleted successfully.");
        cnt.remove();
        cnt.revalidate();
        // Refund the payment
        try {
            RefundCreateParams refundParams =
                RefundCreateParams.builder()
                    .setCharge(rec.getChargeId())
                    .build();
            Refund.create(refundParams);
            ToastBar.showInfoMessage("Payment refunded successfully.");
        } catch (StripeException ex) {
            ex.printStackTrace();
            ToastBar.showErrorMessage("Error refunding payment: " + ex.getMessage());
        }
    });
    NetworkManager.getInstance().addToQueueAndWait(request);
        ...
        cnt.add(BorderLayout.CENTER,BoxLayout.encloseY(
                ...
                BoxLayout.encloseX(lblDate),
                BoxLayout.encloseX(lblProduit),
                BoxLayout.encloseX(lblPrix),
